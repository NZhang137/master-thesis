"""Train one TinyLlama HelpSteer1 DPO expert for the NB12 RQ2 regime.

For every HelpSteer1 prompt this script forms all non-tied within-prompt
response pairs and retains exactly one.  It first maximizes the rating gap on
the target attribute, then prefers the higher chosen rating, and finally
minimizes the summed absolute rating differences on the four non-target
attributes.  Remaining exact ties are resolved reproducibly with the fixed
seed.  ArmoRM is deliberately absent from pair selection and training.

The five NB12 calls must use the same base revision, seed, pair cap, LoRA
configuration, and optimization settings.  Only ``--reward_name`` changes.
The common 5,085-prompt cap gives every expert the same number of optimizer
steps while preventing any prompt from appearing in more than one pair.
"""
...
DEFAULT_DATASET = "nvidia/HelpSteer"
DEFAULT_SPLIT = "train"
DEFAULT_SEED = 137
DEFAULT_MAX_PAIRS = 5085


def _clean_text(value: object, field: str) -> str:
    text = str(value or "").strip()
    if not text:
        raise ValueError(f"HelpSteer row has an empty {field}.")
    return text
...
def _rating(row: Mapping[str, object], attribute: str) -> float:
    try:
        ...
        raise ValueError(f"Invalid {attribute!r} rating in HelpSteer row.") from error
    if not math.isfinite(value):
        raise ValueError(f"Non-finite {attribute!r} rating in HelpSteer row.")
    return value
    ...
    for prompt, group in groups.items():
        for row_a, row_b in combinations(group, 2):
            ...
            if chosen == rejected:
                continue
            target_gap = (
                score_a - score_b if score_a > score_b else score_b - score_a
            )
            other_axes_gap = sum(
                abs(_rating(high, other) - _rating(low, other))
                for other in HELPSTEER_ATTRIBUTES
                if other != attribute
            )
            pairs.append(
                {
                    ...
                    "chosen": chosen,
                    "rejected": rejected,
                    "target_rating_gap": target_gap,
                    "chosen_target_rating": max(score_a, score_b),
                    "other_axes_abs_gap_sum": other_axes_gap,
                }
            )
    return pairs
...
def select_pairs(
    ...
    *,
    ...
    """Choose one ranked pair per prompt, then retain the top ``max_pairs``."""
    if max_pairs < 1:
        raise ValueError("max_pairs must be at least 1.")
    by_prompt: OrderedDict[str, list[Mapping[str, object]]] = OrderedDict()
    for pair in pairs:
        prompt = str(pair["raw_prompt"])
        by_prompt.setdefault(prompt, []).append(pair)

    def rank(pair: Mapping[str, object]) -> tuple[float, float, float]:
        return (
            float(pair["target_rating_gap"]),
            float(pair["chosen_target_rating"]),
            -float(pair["other_axes_abs_gap_sum"]),
        )

    def seeded_tie_key(pair: Mapping[str, object]) -> str:
        return hashlib.sha256(
            f"{seed}|{pair['raw_prompt']}|{pair['pair_id']}".encode("utf-8")
        ).hexdigest()

    per_prompt: list[dict[str, object]] = []
    for prompt, prompt_pairs in by_prompt.items():
        best_rank = max(rank(pair) for pair in prompt_pairs)
        tied = [pair for pair in prompt_pairs if rank(pair) == best_rank]
        # A hash-based final tie break is deterministic even if dataset row
        # order changes, while still being explicitly controlled by ``seed``.
        winner = min(
            tied,
            key=seeded_tie_key,
        )
        per_prompt.append(dict(winner))

    if len(per_prompt) < max_pairs:
        raise ValueError(
            f"Only {len(per_prompt)} prompts with a non-tied pair exist; "
            f"{max_pairs} requested."
        )
    # Apply the same scientific priority at the cross-prompt budget boundary:
    # a 4-2 pair therefore precedes a 3-1 or 2-0 pair.  Only exact ranking
    # ties use the seeded hash.
    selected = sorted(
        per_prompt,
        key=lambda pair: (
            -rank(pair)[0],
    ...
    if len({str(pair["raw_prompt"]) for pair in selected}) != len(selected):
        raise AssertionError("One-pair-per-prompt selection produced duplicate prompts.")
    return selected
...
def format_pairs_for_dpo(
    ...
    for pair in pairs:
        prompt = tokenizer.apply_chat_template(
            [{"role": "user", "content": str(pair["raw_prompt"])}],
            tokenize=False,
        ...
        formatted.append(
            {
                "prompt": prompt,
                "chosen": str(pair["chosen"]) + eos,
                "rejected": str(pair["rejected"]) + eos,
            }
        )
    return formatted
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output_root",
        default="results/dpo_rq2/nb12_helpsteer1_onepair5085_seed137_run1",
    )
...
def main() -> None:
    ...
    if trl_version < Version("0.12") and transformers_version >= Version("4.46"):
        raise RuntimeError(
            ...
            "a signature that conflicts with Transformers >= 4.46. Use the NB12 "
    ...
    print(
        f"[pairs] axis={args.reward_name} candidate_pairs={len(candidates)} "
        f"eligible_prompts={len({p['raw_prompt'] for p in candidates})} "
        f"selected={len(selected)} unique_prompts="
        f"{len({p['raw_prompt'] for p in selected})}"
    )
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("NB12 DPO training requires a CUDA GPU.")
    if not torch.cuda.is_bf16_supported():
        raise RuntimeError("NB12 is fixed to bf16 and requires a bf16-capable GPU.")
    ...
    binding = {
        ...
        "pair_rule": (
            "one pair per prompt; within prompts and at the global budget boundary "
            "maximize target rating gap; then maximize chosen target rating; then "
            "minimize summed absolute gap on non-target axes; remaining ties "
            "resolved by seeded hash"
        ),
        "one_pair_per_prompt": True,
        "pair_seed": args.seed,
        "candidate_pair_count": len(candidates),
        "eligible_prompt_count": len({pair["raw_prompt"] for pair in candidates}),
        "selected_pair_count": len(selected),
        "selected_unique_prompt_count": len(
            {pair["raw_prompt"] for pair in selected}
        ),
